...
| --------------------- | -------------- | --------- | ---------- |
| `url_afiliado` na `LinhaOferta` | `String`; `DS_URL_AFILIADO` NULL vira `""`. Linha com URL vazia (após trim) é rejeitada com `Rejeicao::UrlAfiliadoAusente` em `para_pagina` e na geração; `para_card` não muda | CONTRATO §10.1: oferta sem URL não é gerada. Rejeitar (e logar) em vez de derrubar a leitura; `para_card` fica igual para não mexer nos testes de card | n |
| Quem entra na KVS | Todo card publicado (ativo ou expirado `x:1`), com a URL trimada | MANIFEST §6.7: a chave só sai no expurgo; link `/ir/{id}` já compartilhado de uma expirada continua levando à loja | n |
...
| Idempotência na AWS ("zero escritas") | Segunda execução sem mudança: 0 chunks gravados, 0 removidos, 0 putKey, 0 deleteKey. `manifest.json` e `manifest.prev.json` continuam sendo regravados (`versao` nova) | Comportamento de BSV-11 coberto por teste (`ciclo.rs`); mudar isso é decisão do dono | n |
...
| Escrita na KVS real | `UpdateKeys` em lotes de 50 (puts + deletes), encadeando o `ETag` a partir de `DescribeKeyValueStore` | Regra 4 pede "só o diff"; `PutKey`/`DeleteKey` um a um fariam 30 000 chamadas sequenciais no primeiro ciclo. Quota da AWS: 50 chaves ou 3 MB por chamada | n |
...
| Runtime assíncrono | `ContextoAws` guarda um `tokio` `current_thread` compartilhado (`Arc`) por `PublicadorS3` e `RedirectsKvs`; cada método faz `block_on`. `gerar()` continua síncrono | Regra 8: async só na borda; um runtime por processo, sem threads extras | n |
...
| Dependências | `aws-config` (`behavior-version-latest`), `aws-sdk-s3` e `aws-sdk-cloudfrontkeyvaluestore` sem features padrão (`default-https-client`, `rt-tokio`; `sigv4a` na KVS, que exige SigV4A), `tokio` só com `rt` | Regra 8; sem features padrão evita o hyper 0.14 / rustls 0.21 legados | n |
...
2. The `RedirectsKvs` SHALL implementar `Redirects` com `ListKeys` paginado e `UpdateKeys` em lotes de até 50 com `IfMatch` do `ETag` corrente.  <!-- AWS-02 -->
...
| KVS-04 | P1: KVS | T3 | Pending |
| KVS-05 | P1: KVS | T3 | Pending |
| KVS-06 | P1: KVS | T3 | Pending |
| KVS-07 | P1: KVS | T3 | Pending |
